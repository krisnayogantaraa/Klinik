<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Data Obat</h2>
    </x-slot>

    <div class="max-w-xl mx-auto py-6">
        <form action="{{ route('medicines.update', $medicine->id) }}" method="POST" class="space-y-4 bg-white dark:bg-gray-800 p-6 rounded shadow">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="satuan" name="satuan" class="mt-1 block w-full rounded border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100" required>
                    ...
                    @foreach ($satuanOptions as $option)
                        <option value="{{ $option }}" {{ old('satuan', $medicine->satuan) == $option ? 'selected' : '' }}>
                            {{ ucfirst($option) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                <button type="submit"
                    class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-700">Simpan Perubahan</button>
